...
function Locations() {
  ...
  return <section id="visit" className="relative overflow-hidden bg-vanilla px-5 py-24 md:px-10 md:py-32"><div className="grain absolute inset-0 opacity-10" /><div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="eyebrow text-mango-deep">Find your little happy</p><h2 className="display mt-4 text-5xl leading-[.84] md:text-7xl">Two cities.<br /><span>One bright mood.</span></h2><p className="mt-6 max-w-sm text-sm leading-relaxed text-ink/70">Mango’s is bringing its dessert ritual to people across Bangalore and the UAE.</p></div><div className="grid gap-4 md:grid-cols-2">{cities.map((city) => <Link key={city.slug} to={`/locations/${city.slug}`} aria-label={`Explore Mango's outlets in ${city.name}`} className="group relative min-h-[218px] overflow-hidden rounded-[1.6rem] border border-white/20 p-5 shadow-[0_10px_0_rgba(16,59,43,.16),0_16px_28px_rgba(16,59,43,.1)] transition-transform duration-500 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mango-deep focus-visible:ring-offset-4 sm:min-h-[250px] sm:rounded-[1.8rem] sm:p-6 md:min-h-[285px] md:rounded-[2rem] md:p-7 md:shadow-[0_14px_0_rgba(16,59,43,.18),0_21px_34px_rgba(16,59,43,.12)]" style={city.style}><div aria-hidden="true" className="absolute -right-8 -top-12 h-36 w-36 rounded-full border border-white/20 sm:-right-10 sm:-top-14 sm:h-44 sm:w-44 md:-right-12 md:-top-16 md:h-48 md:w-48" /><div aria-hidden="true" className="absolute bottom-0 left-0 h-[44%] w-full bg-[linear-gradient(180deg,transparent,rgba(0,0,0,.17))]" /><MapPin size={20} className={`relative z-10 sm:h-6 sm:w-6 ${city.accent}`} /><div className="relative z-10 mt-9 sm:mt-12 md:mt-16"><p className={`display text-[2rem] leading-none sm:text-4xl ${city.textTone}`}>{city.name}</p><p className={`mt-2 max-w-[22ch] text-[13px] leading-relaxed sm:mt-3 sm:max-w-[18ch] sm:text-sm ${city.mutedTone}`}>{city.copy}</p></div><span className={`absolute bottom-5 left-5 z-10 inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[.12em] sm:bottom-6 sm:left-6 sm:text-[11px] sm:tracking-[.14em] md:left-7 ${city.textTone}`}>Explore outlets <ArrowUpRight size={14} className="transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></span></Link>)}</div></div></section>;
}
