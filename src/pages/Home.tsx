...
const homeDescription = "Mango's serves DBC (Death by Chocolate), ice creams, waffles, shakes, sundaes and falooda near Kalyan Nagar, CMR Road, Frazer Town and Udupi.";
const localAreas = [
  { name: 'Kalyan Nagar', detail: 'DBC, ice creams, waffles and shakes for your next sweet stop.' },
  { name: 'CMR Road', detail: 'Dessert favourites made for a quick treat or an unhurried catch-up.' },
  { name: 'Frazer Town', detail: 'Sundaes, falooda, shakes and waffles for every kind of craving.' },
  { name: 'Udupi', detail: 'Bright Mango’s desserts, from DBC to ice creams and more.' },
];
const localBusinessSchema = {
  ...
  telephone: '+91 99450 41870',
  servesCuisine: ['Ice Cream', 'Desserts', 'Waffles', 'Milkshakes'],
  areaServed: localAreas.map(({ name }) => ({ '@type': 'Place', name })),
  makesOffer: [
    { '@type': 'Offer', itemOffered: { '@type': 'MenuItem', name: 'DBC (Death by Chocolate)' } },
    { '@type': 'Offer', itemOffered: { '@type': 'MenuItem', name: 'Waffles' } },
    { '@type': 'Offer', itemOffered: { '@type': 'MenuItem', name: 'Ice creams and sundaes' } },
  ],
};
const products = [
...
function Locations() {
  ...
  return <section id="visit" className="relative overflow-hidden bg-vanilla px-5 py-24 md:px-10 md:py-32"><div className="grain absolute inset-0 opacity-10" /><div className="relative mx-auto max-w-7xl"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="eyebrow text-mango-deep">Find your little happy</p><h2 className="display mt-4 text-5xl leading-[.84] md:text-7xl">Two regions.<br /><span>One bright mood.</span></h2><p className="mt-6 max-w-sm text-sm leading-relaxed text-ink/70">Mango’s is bringing its dessert ritual to people across South India and the UAE.</p></div><div className="grid gap-4 md:grid-cols-2">{cities.map((city) => <article key={city.name} className="relative min-h-[218px] overflow-hidden rounded-[1.6rem] border border-white/20 p-5 shadow-[0_10px_0_rgba(16,59,43,.16),0_16px_28px_rgba(16,59,43,.1)] sm:min-h-[250px] sm:rounded-[1.8rem] sm:p-6 md:min-h-[285px] md:rounded-[2rem] md:p-7 md:shadow-[0_14px_0_rgba(16,59,43,.18),0_21px_34px_rgba(16,59,43,.12)]" style={city.style}><div aria-hidden="true" className="absolute -right-8 -top-12 h-36 w-36 rounded-full border border-white/20 sm:-right-10 sm:-top-14 sm:h-44 sm:w-44 md:-right-12 md:-top-16 md:h-48 md:w-48" /><div aria-hidden="true" className="absolute bottom-0 left-0 h-[44%] w-full bg-[linear-gradient(180deg,transparent,rgba(0,0,0,.17))]" /><MapPin size={20} className={`relative z-10 sm:h-6 sm:w-6 ${city.accent}`} /><div className="relative z-10 mt-9 sm:mt-12 md:mt-16"><p className={`display text-[2rem] leading-none sm:text-4xl ${city.textTone}`}>{city.name}</p><p className={`mt-2 max-w-[22ch] text-[13px] leading-relaxed sm:mt-3 sm:max-w-[18ch] sm:text-sm ${city.mutedTone}`}>{city.copy}</p></div><span className={`absolute bottom-5 left-5 z-10 text-[10px] font-black uppercase tracking-[.12em] sm:bottom-6 sm:left-6 sm:text-[11px] sm:tracking-[.14em] md:left-7 ${city.textTone}`}>Details coming soon</span></article>)}</div></div><div className="mt-12 rounded-[2rem] border border-ink/10 bg-cream/70 p-6 shadow-[0_18px_40px_rgba(16,59,43,.08)] md:mt-16 md:p-9"><div className="max-w-2xl"><p className="eyebrow text-mango-deep">Dessert spots near you</p><h3 className="display mt-3 text-4xl leading-[.9] text-ink md:text-6xl">Find DBC, waffles and ice cream <i className="font-light">near you.</i></h3><p className="mt-4 max-w-xl text-sm leading-relaxed text-ink/70 md:text-base">Looking for a DBC dessert, a waffle, or a cold scoop? Discover Mango’s in the neighbourhoods below.</p></div><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{localAreas.map((area) => <article key={area.name} className="rounded-[1.35rem] border border-ink/10 bg-vanilla p-5"><MapPin size={18} className="text-mango-deep" /><h4 className="mt-5 text-lg font-bold text-ink">Mango’s near {area.name}</h4><p className="mt-2 text-sm leading-relaxed text-ink/65">{area.detail}</p></article>)}</div></div></div></section>;
}
...
export default function Home() { return <main id="top" className="overflow-x-clip"><Helmet><title>Mango's | DBC, Ice Creams, Waffles & Shakes</title><meta name="description" content={homeDescription} /><meta property="og:title" content="Mango's | DBC, Ice Creams, Waffles & Shakes" /><meta property="og:description" content={homeDescription} /><meta property="og:site_name" content="Mango's" /><meta property="og:type" content="website" /><meta property="og:locale" content="en_IN" /><script type="application/ld+json">{JSON.stringify(localBusinessSchema)}</script></Helmet><Intro /><Hero /><MangoMarquee /><LiquidPortal /><Story /><ProductUniverse /><Experience /><SocialProof /><Locations /><Franchise /><Footer /><BackToTop /></main>; }
